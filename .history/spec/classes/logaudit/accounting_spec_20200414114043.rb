...
describe 'cis_hardening::logaudit::accounting' do
  on_supported_os.each do |os, os_facts|
    context "on #{os}" do
      ...
      it { is_expected.to contain_file_line('faillock').with(
        'ensure' => 'present',
        'path'   => '/etc/audit/audit.rules',
      ...
      it { is_expected.to contain_file_line('utmp_entry').with(
        'ensure' => 'present',
        'path'   => '/etc/audit/audit.rules',
        'line'   => '-w /var/run/utmp -p wa -k session',
      )}

      it { is_expected.to contain_file_line('wtmp_entry').with(
        'ensure' => 'present',
        'path'   => '/etc/audit/audit.rules',
